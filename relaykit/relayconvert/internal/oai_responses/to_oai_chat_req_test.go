...
func TestResponsesRequestToChatCompletionsRequestCustomToolCallPreservesRawShape(t *testing.T) {
	...
	toolCalls := got.Messages[0].ParseToolCalls()
	require.Len(t, toolCalls, 1)
	// The converter preserves the native "custom" type; upstreams that only
	// accept "function" (e.g. opencode Console Go) are normalized at the
	// channel adaptor layer where the upstream identity is known.
